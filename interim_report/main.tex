...
The main technologies are C, OpenMP, LLVM/Clang, Python, CSV files, and LaTeX. On macOS, Homebrew LLVM/Clang and \texttt{libomp} are used because Apple Clang does not provide the needed OpenMP support by default. On the Intel desktop, Clang 22.1.8 links against \texttt{libomp.so}. Python scripts drive experiments, normalize outputs, collect metadata, summarize results, compare machines, and generate plots. The report itself is compiled with Tectonic. The source code repository is available at \url{https://github.com/dontoperatesystem32/memory_bound_benchmark}.

The Intel desktop experiments are run remotely rather than at the physical machine. The access path uses a Raspberry Pi on the home network for VPN access, Wake-on-LAN support, and SSH into the desktop once it is powered on. This setup is not a research contribution, but it is useful project infrastructure: it makes the x86-64 pilot platform available during the course without requiring physical access and helps keep the experiment workflow repeatable.
...
\begin{tikzpicture}[x=1cm,y=0.036cm]
  ...
  \filldraw[blue!60, draw=black] (4.45,105.5) rectangle (4.80,110.0);
  \node[anchor=west] at (4.92,108) {\small Mac M4};
  \filldraw[orange!75, draw=black] (4.45,95.5) rectangle (4.80,100.0);
  \node[anchor=west] at (4.92,98) {\small Intel i5-12400F};
\end{tikzpicture}
...
\begin{tikzpicture}[x=0.18cm,y=0.22cm]
  ...
  \draw[blue!70, line width=1.4pt] (18,16.8) -- (20.5,16.8);
  \filldraw[blue!70, draw=black] (19.25,16.8) circle (0.32);
  \node[anchor=west] at (21.0,16.8) {\small Mac M4};
  \draw[orange!85, line width=1.4pt] (18,15.2) -- (20.5,15.2);
  \filldraw[orange!85, draw=black] (19.25,15.2) circle (0.32);
  \node[anchor=west] at (21.0,15.2) {\small Intel i5-12400F};
\end{tikzpicture}
...
Measurement noise is another important risk. Laptop thermals, background processes, power management, OS scheduler behavior, and OpenMP thread placement can affect results. This risk is visible in the Mac full sweep, where 11 of 196 groups exceeded the 10 percent coefficient-of-variation threshold. Most of those warnings occur in four-thread stencil or strided cases. One practical reason is that the MacBook Air was also being used interactively during parts of the measurement period, with normal desktop activity such as a browser, music, and other background processes present. The Intel desktop, by contrast, was accessed remotely while no one was physically using it, so the benchmark process had a cleaner and more idle run environment. The mitigation is to report medians, track coefficient of variation, avoid hiding unstable groups, document run-environment differences, and avoid interpreting variation differences as architectural properties.
